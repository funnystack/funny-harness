...
| `docs/architecture.md`              | `{项目根目录}/docs/architecture.md`              | 架构模式总览（引用子文件） |
| `docs/architecture/tech-stack.md`   | `{项目根目录}/docs/architecture/tech-stack.md`   | 技术栈详情                 |
| `docs/architecture/api-design.md`   | `{项目根目录}/docs/architecture/api-design.md`   | API 设计模式与入口清单     |
| `docs/architecture/database.md`     | `{项目根目录}/docs/architecture/database.md`     | 数据库规范与表字段清单     |
| `docs/architecture/jobs-design.md`  | `{项目根目录}/docs/architecture/jobs-design.md`  | 定时任务设计               |
| `docs/architecture/consumer.md`     | `{项目根目录}/docs/architecture/consumer.md`     | 消息消费者入口清单         |
| `docs/architecture/testing.md`      | `{项目根目录}/docs/architecture/testing.md`      | 测试规范                   |
...
**产出**: 除了填充 `docs/architecture/tech-stack.md` 中的 RPC 信息外，将详细分析追加到 `docs/architecture.md` 的"远程调用约定"部分。

**注意**: 不要读取 `application-*.yml`、`*.properties`、`*.env` 等配置文件的具体内容（可能包含密码），只通过代码中的 import 和注解推断。

**产出**: 读取本 Skill 的 `docs/architecture/tech-stack.md` 模板，填充扫描结果后写入目标项目的 `docs/architecture/tech-stack.md`。
...
3. 对每个定时任务，收集:
   ...
   - 特殊标注:
     ...
     **产出**: 读取本 Skill 的 `docs/architecture/jobs-design.md` 模板，填充定时任务规范和入口清单后写入目标项目的 `docs/architecture/jobs-design.md`。同时确认 `docs/architecture.md` 中已引用 `@docs/architecture/jobs-design.md`。
...
3. 对每个消费者，收集:
   ...
   - 特殊标注:
     ...
     **产出**: 读取本 Skill 的 `docs/architecture/consumer.md` 模板，填充消息消费者规范和入口清单后写入目标项目的 `docs/architecture/consumer.md`。同时确认 `docs/architecture.md` 中已引用 `@docs/architecture/consumer.md`。
...
**产出**: 将事务模式分析追加到 `docs/architecture.md` 的对应位置。
...
**产出**: 读取本 Skill 的 `docs/architecture.md` 模板，填充 Step 3-7 的全部扫描结果（含 Step 5.5 定时任务）后写入目标项目的 `docs/architecture.md`。
...
6. **半自动确认**: Step 8-10 的产出需要人工确认。扫描完成后统一提示用户查看确认，不要每一步都打断。
7. **@ 引用语法**: AGENTS.md 中的 `@docs/xxx.md` 是 Claude 的文件引用语法，Claude 读取 AGENTS.md 时会自动加载引用的文件内容。
8. **CLAUDE.md 必须引用 AGENTS.md**: 生成的 `CLAUDE.md` 第一行必须是 `@AGENTS.md`，这是 Claude 加载项目规则的入口。Step 1 中有校验步骤，如果校验失败则整个流程中断。无论模板如何变更，这个约束不可违反。
